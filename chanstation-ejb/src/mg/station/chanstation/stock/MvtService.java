...
public class MvtService {
    ...
    protected MvtStock createStockLine(Cuve cuve,Carburant carburant,int typeMov) throws Exception {
        ...
        mvtStock.setId_cuve(cuve.getId_cuve());
    ...
    protected MvtStock getLastMvt(Cuve cuve,Connection c) throws Exception {
        if (c==null) c = new UtilDB().GetConn();
        MvtStock[] mvtStocks = ((MvtStock[]) CGenUtil.rechercher(new MvtStock(),null,null," and daty=(SELECT MAX(daty) FROM stock WHERE id_cuve = '"+ cuve.getId_cuve() +"') and id_cuve = '"+cuve.getId_cuve()+"'"));
        MvtStock ret = null;
