...
<%
try{
    EjbStation2 ejb = EjbServiceProvider.getEjbLocalServer();
    PrelevementCpl[] prels = ejb.getPrelevements();
    for(int i=0;i < prels.length;i++){
        if((i+1)%2 == 0){
            out.println("<td><a href=\"myStation/prelevement?idFacture="+prels[i].getId()+"\">"+prels[i].getId()+"-"+prels[i].getDaty()+"-"+prels[i].getIdPompe()+"</a></td>");
        }
    }
    }catch(Exception e){
        e.printStackTrace();
